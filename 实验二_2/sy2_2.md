...
```  
4. 布局界面效果如图  
![](/实验二_2/pic/屏幕截图%202023-05-18%20013307.png)  
...
1. 将此分析器添加为 MainActivity.kt 中的内部类。分析器会记录图像的平均亮度。如需创建分析器，我们会替换实现 ImageAnalysis.Analyzer 接口的类中的 analyze 函数。  
...
```
运行应用，它会大约每秒在 logcat 中生成一个类似于下面的消息：
```
...
```
4. 将 Preview + VideoCapture 用例绑定到生命周期相机。仍在 startCamera() 内，将 cameraProvider.bindToLifecycle() 调用替换为以下代码：  
```kotlin
// Bind use cases to camera
cameraProvider.bindToLifecycle(this, cameraSelector, preview, videoCapture)
```
5. 构建并运行项目  
6. 按“START CAPTURE”按钮。注意，图片说明会变为“STOP CAPTURE”  
![](/实验二_2/pic/1685524059686.jpg)  
按“STOP CAPTURE”按钮（和 start capture 按钮是同一个按钮）。  
1. 视频录制测试可以看到视频被成功保存至媒体库  
